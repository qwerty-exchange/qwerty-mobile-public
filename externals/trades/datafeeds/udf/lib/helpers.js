import { getHours, getMinutes, getTime, set, subHours, subMinutes } from 'date-fns';
...
const roundToNearestHour = (timestamp, resolution) => {
    ...
    return getTime(set(subHours(timestamp, hoursToSubtract), {
        minutes: 0,
        seconds: 0,
        milliseconds: 0
    }));
};
const roundToNearestMinute = (timestamp, resolution) => {
    const minute = Number(resolution);
    const minutes = getMinutes(timestamp);
    const minsToSubtract = minutes % minute;
    return getTime(set(subMinutes(timestamp, minsToSubtract), {
        seconds: 0,
        milliseconds: 0
    }));
};
export const roundTimestampByResolution = (resolution, timestamp) => {
