import { derivativePriceToChainPrice, formatAmountToAllowableAmount } from '@injectivelabs/sdk-ts';
import {
...
export const calculateMargin = ({
  ...
  const allowableMargin = formatAmountToAllowableAmount(marginInWei.toFixed(), tensMultiplier);
...
export const calculateLiquidationPrice = ({
  ...
  const numerator = isBuy
    ? new BigNumberInBase(notionalWithLeverage).minus(new BigNumberInBase(price).times(quantity))
    : new BigNumberInBase(notionalWithLeverage).plus(new BigNumberInBase(price).times(quantity));
